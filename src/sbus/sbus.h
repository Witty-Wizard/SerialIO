...
private:
  bool header_detected_sbus = false; ///< Flag indicating whether the SBUS header has been detected
  int prev_buffer_sbus;              ///< Previous buffer for SBUS communication
  int buffer_sbus;                   ///< Buffer for SBUS communication
  int rx_index;                      ///< Index for RX buffer
  uint8_t data_rx[SBUS_MAX_PACKET_SIZE]; ///< RX data buffer for SBUS communication
